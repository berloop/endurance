...
import { Activity, BoxIcon, Check, CircleDot, Cpu, Disc3Icon, ChevronDown, ChevronUp, Settings } from 'lucide-react';
...
const DebugWormhole: React.FC<{ className?: string }> = ({ className = '' }) => {
    ...
    if (renderMode === 'raytraced' && textures.galaxy) {
      ...
      const rayTracerMaterial = new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          uRingColor: { value: new THREE.Vector3(advancedParams.ringColor.r, advancedParams.ringColor.g, advancedParams.ringColor.b) },
  ...
  }, [parameters, textures, renderMode, advancedParams, cameraPosition, rayTracingVertexShader, rayTracingFragmentShader]);
    ...
    const animate = () => {
      ...
      if (renderMode === 'geometry') {
        ...
        if (rayTracer && (rayTracer as any).rayTracerMaterial && cameraRef.current) {
          ...
          material.uniforms.uWarpingDistance.value = advancedParams.warpingDistance;
          ...
          material.uniforms.uRingColor.value.set(advancedParams.ringColor.r, advancedParams.ringColor.g, advancedParams.ringColor.b);
      ...
      <div className="absolute top-4 left-4 bg-neutral-950 backdrop-blur-sm rounded-xs p-4 text-white max-w-xs max-h-[90vh] overflow-y-auto scrollbar-none">
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-gray-600">
            <h4 className="text-md font-medium mb-4 text-gray-300">Camera Position</h4>
        ...
        {showAdvanced && (
          <div className="mt-4 space-y-6 border-t border-gray-600 pt-4">
            <div>
              <label className="block text-sm mb-2">Rotation Speed: {advancedParams.rotationSpeed.toFixed(2)}</label>
              <Slider
                value={[advancedParams.rotationSpeed]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, rotationSpeed: value[0] }))}
                min={0}
                max={1.0}
                step={0.01}
              >
            ...
            <div>
              <label className="block text-sm mb-2">Warping Distance: {advancedParams.warpingDistance.toFixed(1)}</label>
              <Slider
                value={[advancedParams.warpingDistance]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, warpingDistance: value[0] }))}
                min={0.1}
                max={5.0}
                step={0.1}
              >
  ...
  <div className="grid grid-cols-2 gap-2">
    <Button
      size="sm"
      variant={advancedParams.rotationMode === 0 ? 'default' : 'secondary'}
      onClick={() => setAdvancedParams(prev => ({ ...prev, rotationMode: 0 }))}
    >
      Oscillating
    </Button>
    <Button
      size="sm"
      variant={advancedParams.rotationMode === 1 ? 'default' : 'secondary'}
      onClick={() => setAdvancedParams(prev => ({ ...prev, rotationMode: 1 }))}
    >
      Bounded
    </Button>
    <Button
      size="sm"
      variant={advancedParams.rotationMode === 2 ? 'default' : 'secondary'}
      onClick={() => setAdvancedParams(prev => ({ ...prev, rotationMode: 2 }))}
    >
      Linear
    </Button>
    <Button
      size="sm"
      variant={advancedParams.rotationMode === 3 ? 'default' : 'secondary'}
      onClick={() => setAdvancedParams(prev => ({ ...prev, rotationMode: 3 }))}
    >
      Spiral
    </Button>
  </div>
            ...
            <div>
              <label className="block text-sm mb-2">Ring Radius: {advancedParams.ringRadius.toFixed(2)}</label>
              <Slider
                value={[advancedParams.ringRadius]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, ringRadius: value[0] }))}
                min={0.01}
                max={1.0}
                step={0.01}
              >
            ...
            <div>
              <label className="block text-sm mb-2">Ring Sharpness: {advancedParams.ringSharpness.toFixed(1)}</label>
              <Slider
                value={[advancedParams.ringSharpness]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, ringSharpness: value[0] }))}
                min={1.0}
                max={50.0}
                step={0.5}
              >
            ...
            <div>
              <label className="block text-sm mb-2">Ring Intensity: {advancedParams.ringIntensity.toFixed(2)}</label>
              <Slider
                value={[advancedParams.ringIntensity]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, ringIntensity: value[0] }))}
                min={0}
                max={1.0}
                step={0.01}
              >
            ...
            <div>
              <label className="block text-sm mb-2">Zoom: {advancedParams.zoom.toFixed(2)}</label>
              <Slider
                value={[advancedParams.zoom]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, zoom: value[0] }))}
                min={0.5}
                max={3.0}
                step={0.01}
              >
            ...
            <div>
              <label className="block text-sm mb-2">Max Steps: {advancedParams.maxSteps}</label>
              <Slider
                value={[advancedParams.maxSteps]}
                onValueChange={(value) => setAdvancedParams(prev => ({ ...prev, maxSteps: value[0] }))}
                min={100}
                max={500}
                step={10}
              >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs mb-1 text-gray-400">Red: {advancedParams.ringColor.r.toFixed(2)}</label>
                <Slider
              ...
              <div>
                <label className="block text-xs mb-1 text-gray-400">Green: {advancedParams.ringColor.g.toFixed(2)}</label>
                <Slider
              ...
              <div>
                <label className="block text-xs mb-1 text-gray-400">Blue: {advancedParams.ringColor.b.toFixed(2)}</label>
                <Slider
