...
const MusicControls: React.FC<MusicControlsProps> = ({ className = "" }) => {
  ...
  const tracks = [
    { name: 'Imbere Yanje | Judah Earl & Odeta', file: '/music/imbere.yanje.judah.earl.odeta.mp3' },
  ...
  return (
    <div className={`bg-neutral-950/20 backdrop-blur-sm rounded-sm p-4 text-white max-w-xs ${className}`}>
        ...
        <div className="grid grid-cols-1 gap-1">
          {tracks.map((track, index) => (
            <Button
              key={index}
              size="sm"
              variant={currentTrack === index ? "ghost" : "ghost"}
      ...
      {/* Current Track Info */}
      <div className="mb-3 text-xs text-neutral-500 font-mono tracking-tight">
